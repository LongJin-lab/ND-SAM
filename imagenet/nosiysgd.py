...
from differential_privacy.dpsgd import DPSGD
from differential_privacy.sgdmdp import MomentumDP
...
logger = Logger('dpsgd_imagenet_r18_dp.txt', title='imagenet')
...
def main():
    ...
    # base_optimizer = ND
    # optimizer = NDSAM(model.parameters(), base_optimizer, rho=args.rho, adaptive=args.adaptive, lr=args.lr,momentum=args.momentum,weight_decay=args.weight_decay)
    # optimizer = torch.optim.SGD(model.parameters(), args.lr,momentum=args.momentum, weight_decay=args.weight_decay)         
    # optimizer = DPSGD(model.parameters(), lr=args.lr, batch_size=args.batch_size//args.microbatches, momentum=args.momentum, weight_decay=args.weight_decay, dampening=args.dampening)
    optimizer = MomentumDP(model.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
...
##### 
    for epoch in range(args.start_epoch, args.epochs):
        ...
        logger.append(['{:.3f}'.format(val_loss), '{:.3f}'.format(best_prec1), '{:.3f}'.format(prec5)])
...
def save_checkpoint(state, is_best, filename='sgdcheckpoint.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'sgd_model_best.pth.tar')
